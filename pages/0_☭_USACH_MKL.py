...
translate = {"knee": "knee", "hip": "hip", "ankle": "foot", "trunk": "torso"}
...
def compare(dfq, dfa, parts):
    ...
    samp = st.number_input(
        "Ingresar la frecuencia de muestreo de QTM",
        value=88,
    ...
    adv = st.toggle("Modo avanzado", key="advanced")
    ...
    for part in parts:
        ...
        if samp != 120:
            ...
            qtmNew = pd.DataFrame({"Frame": index, f"{part}": qtmSamp})
        ...
        if st.checkbox(f"¿Invertir {part} QTM?"):
            qtm = -qtm
        number = st.number_input(
            "Ingresar desfase QTM",
            value=0,
            placeholder="Type a number...",
            min_value=-360,
            max_value=360,
            step=90,
            key=f"{part}-desfase-qtm",
        )
        qtm = qtm + number
        ...
        values = st.slider(
            ...
            int(qtm.index[0]),
            int(qtm.index[-1]),
            (int(qtm.index[0]), int(qtm.index[-1])),
        ...
        st.markdown("###### Señal original QTM")
        st.line_chart(dfo)

        if st.checkbox(f"¿Invertir {part} ABMA?"):
            abma = -abma
        number = st.number_input(
            "Ingresar desfase ABMA",
            value=0,
            placeholder="Type a number...",
            min_value=-360,
            max_value=360,
            step=90,
            key=f"{part}-desfase-abma",
        )
        abma = abma + number
        ...
        values = st.slider(
            ...
            int(abma.index[0]),
            int(abma.index[-1]),
            (int(abma.index[0]), int(abma.index[-1])),
        ...
        st.markdown("###### Señal original ABMA")
        st.line_chart(dfo)
        ...
        try:
            ...
            st.markdown("###### Gráficos de las señales sincronizadas")
            st.line_chart(dft)
            ...
            if adv:
                ...
                YA, YB = (
                    np.array(randomizeM(np.array(qtmc), 100)),
